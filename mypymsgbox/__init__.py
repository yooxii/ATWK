...
def __fillablebox(msg, title="", default="", rows=1, mask=None, root=None, timeout=None):
    """
    ...
    global boxRoot, __enterboxText, __enterboxDefaultText, __rows
    global cancelButton, entryWidget, textWidget, okButton
    ...
    if __rows <= 1:
        ...
        # -----------textWidget --------------------------------------
        textWidget = tk.Text(textFrame, width=40, height=__rows)
        _bindArrows(textWidget, skipArrowKeys=True)
        textWidget.configure(font=(MONOSPACE_FONT_FAMILY, MONOSPACE_FONT_SIZE))
        ...
        if __enterboxDefaultText != "":
            ...
            textWidget.tag_add(tk.SEL, "1.0", tk.END)
    ...
    if __rows <= 1:
        ...
        textWidget.focus_force()  # put the focus on the textWidget
    boxRoot.deiconify()
    if timeout is not None:
        boxRoot.after(timeout, timeoutBoxRoot)
...
def __enterboxGetText(event):
    ...
    if __rows <= 1:
        ...
        __enterboxText = textWidget.get("1.0", tk.END)
